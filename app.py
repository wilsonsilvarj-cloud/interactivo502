...
BACKGROUND_IMAGE_FILE = "Ana Clara.png"
...
def dia_de_hoje_util(dia_padrao: str = "segunda") -> str:
    ...
    return WEEKDAY_TO_DIA.get(hoje_brasil.weekday(), dia_padrao)


def dia_de_amanha_util(dia_padrao: str = "terca") -> str:
    amanha_brasil = datetime.now(ZoneInfo("America/Sao_Paulo")).date() + timedelta(days=1)
    return WEEKDAY_TO_DIA.get(amanha_brasil.weekday(), dia_padrao)
...
def detectar_dia(texto: str, dia_padrao: str = "segunda") -> str:
    ...
    if "amanha" in txt:
        return dia_de_amanha_util(dia_padrao)
    if "segunda" in txt:
        return "segunda"
    if "terca" in txt:
        return "terca"
...
def detectar_dias(pergunta: str) -> list[str]:
    ...
    if "amanha" in txt and not dias:
        dias.append(dia_de_amanha_util("terca"))

    if "hoje" in txt and not dias:
        dias.append(dia_de_hoje_util("segunda"))
...
def imagem_para_base64(caminho: Path | None) -> str:
    """Converte uma imagem local para base64 para usar como fundo da interface."""
...
def responder_prova_modo_basico(pergunta: str) -> str | None:
    ...
    # Correção: perguntas como "que prova tenho amanhã?", "prova depois de amanhã"
    # ou "prova ontem" devem consultar o calendário de provas, não o horário de aulas.
    if any(t in pergunta_norm for t in ["amanha", "depois de amanha", "ontem", "hoje"]):
        ...
        return "Não encontrei prova cadastrada para esse dia no calendário."
...
# CABEÇALHO
# =====================================================
fundo_path = resolve_image_path(BACKGROUND_IMAGE_FILE)
fundo_b64 = imagem_para_base64(fundo_path)
...
if fundo_b64:
    fundo_css = (
        ...
        'url("data:image/png;base64,' + fundo_b64 + '");\n'
...
div.stButton > button {
    ...
    width: 100% !important;
    padding: 8.0rem 7.2rem !important;
}
